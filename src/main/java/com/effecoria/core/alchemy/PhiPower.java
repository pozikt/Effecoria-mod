...
public final class PhiPower {
    ...
    /**
     * MVP stub for future load accounting — currently only verifies a provider exists.
     * Returns true if power is available (does not drain fuel).
     */
        ...
        for (int dy = -SCAN_RADIUS; dy <= SCAN_RADIUS; dy++) {
            for (int dx = -SCAN_RADIUS; dx <= SCAN_RADIUS; dx++) {
                for (int dz = -SCAN_RADIUS; dz <= SCAN_RADIUS; dz++) {
                    ...
                    if (!hasLineOfSight(level, consumerPos, cursor.immutable())) {
                        continue;
                    }
