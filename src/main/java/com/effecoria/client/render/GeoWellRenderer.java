...
/** Draws the assembled Geo Well as one textured 3×3×3 cube centered on the core. */
public final class GeoWellRenderer implements BlockEntityRenderer<GeoWellBlockEntity> {
    private static final ResourceLocation HULL =
            ResourceLocation.fromNamespaceAndPath(EffecoriaMod.MOD_ID, "textures/block/geo_well_hull.png");
    private static final ResourceLocation HULL_ON =
            ResourceLocation.fromNamespaceAndPath(EffecoriaMod.MOD_ID, "textures/block/geo_well_hull_on.png");

    public GeoWellRenderer(BlockEntityRendererProvider.Context context) {}

    @Override
    public void render(
            GeoWellBlockEntity be,
            float partialTick,
            PoseStack poseStack,
            MultiBufferSource buffer,
            int packedLight,
            int packedOverlay) {
        if (!be.getBlockState().getValue(GeoWellBlock.FORMED) && !be.isFormed()) {
            return;
        }

        boolean lit = be.getBlockState().getValue(GeoWellBlock.LIT) || be.supplying();
    ...
    @Override
    public boolean shouldRenderOffScreen(GeoWellBlockEntity be) {
        return be.isFormed() || be.getBlockState().getValue(GeoWellBlock.FORMED);
    }
    ...
    @Override
    public AABB getRenderBoundingBox(GeoWellBlockEntity be) {
