...
public class SpellRadialScreen extends Screen {
    ...
    private void rebuildLayout() {
        if (minecraft == null || minecraft.player == null) {
            layout = new SpellRadialLayout.Layout(SpellRadialLayout.BASE_DEAD_ZONE * 0.7f, 0.7f, java.util.List.of());
            return;
        }
        PlayerPsiData data = minecraft.player.getData(ModAttachments.PSI.get());
        layout = SpellRadialLayout.build(data, this.width, this.height);
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (SpellRadialLayout.Ring ring : layout.rings()) {
            for (SpellRadialLayout.Sector sector : ring.sectors()) {
                ...
                int iconSize = SpellRadialLayout.iconSizeFor(sector, scale);
                drawSpellIcon(graphics, sector.spellId(), lx, ly, iconSize);
            }
        }
        RenderSystem.disableBlend();

        int titleY = cy - Math.round(6 * scale);
        graphics.drawCenteredString(this.font, this.title, cx, titleY, 0xFFFFFF);

        int footerY = this.height - Math.round(28 * scale);
        if (hovered != null) {
            ...
            graphics.drawCenteredString(this.font, spellName, cx, footerY - 12, 0xFFFFFF);
