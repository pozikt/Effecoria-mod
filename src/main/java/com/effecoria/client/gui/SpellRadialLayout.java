...
public final class SpellRadialLayout {
    /** Base layout radii at scale 1.0 — multiply by {@link #menuScale(int, int)} for GUI. */
    public static final float BASE_DEAD_ZONE = 22f;
    public static final float BASE_FAVORITES_OUTER = 54f;
    public static final float BASE_RING_THICKNESS = 30f;
    ...
    public record Layout(float deadZone, float scale, List<Ring> rings) {}

    private SpellRadialLayout() {}

    /** Scale menu to fit smaller screens — keeps the wheel compact. */
    public static float menuScale(int screenWidth, int screenHeight) {
        float minDim = Math.min(screenWidth, screenHeight);
        return Mth.clamp(minDim / 420f, 0.62f, 0.88f);
    }
    ...
    public static Layout build(PlayerPsiData data, int screenWidth, int screenHeight) {
        ...
        float deadZone = BASE_DEAD_ZONE * scale;
        float favoritesOuter = BASE_FAVORITES_OUTER * scale;
        float ringThickness = BASE_RING_THICKNESS * scale;
        ...
        float inner = favorites.isEmpty() ? deadZone : favoritesOuter;
        for (RadialCategory category : RadialCategory.outerRingOrder()) {
            List<IndexedSpell> spells = byCategory.get(category);
            if (spells.isEmpty()) {
                continue;
            }
            float outer = inner + ringThickness;
            rings.add(buildRing(category.langKey(), colorFor(category), inner, outer, spells));
            inner = outer;
        }

        return new Layout(deadZone, scale, List.copyOf(rings));
    }

    public static int iconSizeFor(Sector sector, float scale) {
        float thickness = sector.thickness();
        int fromRing = Math.round(thickness * 0.58f);
        int scaledCap = Math.round(20f * scale);
        return Mth.clamp(fromRing, 14, Math.max(14, scaledCap));
    }
